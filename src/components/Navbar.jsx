...
const Navbar = () => {
  ...
  const handleLocationChange = (city) => {
    ...
    window.location.reload(); // refresh products based on city
  };
  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
      <div className="flex items-center max-w-7xl mx-auto justify-between px-4 py-3">
          ...
          {showLocationList && (
            <div
              className={`
                ...
                ${window.innerWidth < 640 ? "left-1/2 -translate-x-1/2" : "left-0"}
              `}
            >
              {cities.map((city) => (
                <div
                  ...
                  className={`px-4 py-2 cursor-pointer hover:bg-green-50 ${
                    location === city ? "bg-green-100 font-semibold" : ""
                  }`}
                >
                  {city}
                </div>
        ...
        {/* Desktop Search */}
        <div className="hidden lg:flex flex-1 justify-center relative mx-6">
          <form onSubmit={handleSearch} className="w-full max-w-xl" autoComplete="off">
            <div className="flex items-center border border-gray-400 rounded-full overflow-hidden">
              <div className="pl-3 text-gray-400 cursor-pointer" onClick={handleSearch}>
                <FaSearch />
              </div>
              <input
                type="text"
